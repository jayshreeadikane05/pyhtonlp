...
# Assuming root is your parsed HTML document
...
if contenttype == 'Webinar':
    ...
    # Check if the carousel section exists and should not be skipped
    if carousel_section and not should_skip_element(carousel_section):
        # Only select the 'active' item
        active_item = carousel_section.find('div', {'class': 'item active'})
        
        if active_item:
            # Find all the images within the active item
            active_images = active_item.find_all('img')
            ...
            for i, img in enumerate(active_images):
                ...
                # Set the image path and download the image
                image_path = os.path.join(image_folder, f'{i + 1}.png')
                download_image(img_url, image_path)
                
                # Update the img 'src' attribute to point to the relative path
                img['src'] = f'images/{i + 1}.png'
            
            # Convert the modified active item back to an HTML fragment
            active_item_fragment = html.fragment_fromstring(str(active_item), parser)
            print(active_item_fragment)
            
            # Append the active item fragment to the parent section
            parent.append(active_item_fragment)
...
both file is same we need when is contenttype is webinar that time 
                    carousel_section = soup.find('div', {'class': 'carouselSpeakerSection'})
 target_section = root.xpath('//*[@id="mainBodyCopy3"]')
                    if target_section:
                        parent = target_section[0].getparent()
...
if pdflink:
    ...
    if input_pdf:
        if contenttype == 'Webinar':
            # For Webinar, directly set the pdflink
            input_pdf[0].attrib['value'] = pdflink
            print(f'Success: Updated input value to {pdflink} for Webinar')
        else:
            # For other content types, set the local PDF path
            input_pdf[0].attrib['value'] = f'pdf/{pdf_filename}'
            print(f'Success: Updated input value to pdf/{pdf_filename}')
 ...
 if fullbanner_img and 'display: none' not in (fullbanner_img.get('style') or ''):
                        ...
                        if fullbanner_url:
                            ...
                            jumbotron_div = root.xpath('//div[@class="jumbotron"]')
                            if jumbotron_div:   
                                # Set the background URL for the jumbotron div
                                jumbotron_div[0].attrib['style'] = (
                                     f"background: url('images/{os.path.basename(fullbanner_filename)}{image_extension}') no-repeat; "
                                    "background-size: cover; background-color: #0078D7;")
                    else:
                        fullbanner_url = None
